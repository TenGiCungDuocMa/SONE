        ...
        try {
            ...
            for (const kuro of allKuros) {
                if (kuro.participants && Array.isArray(kuro.participants)) {
                    ...
                    const isNewStructure = kuro.participants.every(participant =>
            ...
            this.logger.log(`Successfully updated ${updatedCount} documents with new participants structure`);
            return {
                success: true,
                message: `Successfully updated ${updatedCount} documents with new participants structure`
            };
        } catch (error) {
            this.logger.error(`Error updating participants structure: ${error.message}`);
            throw error;
        }
    }
}
