...
export class KuroService implements OnModuleInit {
    ...
    private async checkForUnprocessedWinners() {
        try {
            ...
            const previousRound = await this.kuroModel.findOne({ roundId: previousRoundId, kuroContractAddress: this.configService.get<string>('KURO_ADDRESS') }).lean();

            if (previousRound &&
                (previousRound.status === KuroStatus.DRAWN || previousRound.status === KuroStatus.CANCELLED) &&
                ...
                this.logger.log(`Detected unannounced winner of round ${previousRoundId}: ${previousRound.winner}`);
                ...
                setTimeout(() => {
                    ...
                    this.logger.log(`Completed processing winner of round ${previousRoundId}`);
                }, 15000);
            }
        } catch (error) {
            this.logger.error(`Error checking for unprocessed winners: ${error.message}`);
            this.isProcessingWinner = false;
        }
    }
    ...
    async getLatestKuroData() {
        try {
            ...
            else if (previousRound.winner == "0x0000000000000000000000000000000000000000" && (previousRound.status == KuroStatus.DRAWN || previousRound.status == KuroStatus.OPEN)) {
                roundId = previousRoundId
            } else {
                roundId = currentRoundId
            }

            const kuroData = await this.kuroModel.findOne({ roundId: roundId, kuroContractAddress: this.configService.get<string>('KURO_ADDRESS') }).lean();
    ...
    async fetchPoolKuro() {
        try {
            ...
            const participants = await this.depositTrackerService.trackDeposit(Number(roundId));
            this.logger.log(`[KURO_SERVICE] Participants: ${participants}`);

            await this.kuroModel.findOneAndUpdate({ roundId: roundId, kuroContractAddress: this.configService.get<string>('KURO_ADDRESS') }, {
                status,
                startTime,
                endTime,
                drawnAt,
            ...
            if (status === KuroStatus.DRAWN &&
                winner !== '0x0000000000000000000000000000000000000000' &&
                !this.processedWinners.has(roundId.toString()) &&
                !this.isProcessingWinner) {


                this.logger.log(`Round ${roundId} just completed with winner: ${winner}`);
            }
    ...
    async fetchPoolKuroByRoundId(roundId: string) {
        const kuro = await this.kuroModel.findOne({ roundId: roundId, kuroContractAddress: this.configService.get<string>('KURO_ADDRESS') }).lean();
        return kuro;
    }
    ...
    async updateWinnerClaimed(roundId: string, txHash: string, userAddress: string) {
        try {
            ...
            const round = await this.kuroModel.findOne({ roundId, kuroContractAddress: this.configService.get<string>('KURO_ADDRESS') }).lean();
            if (!round) {
            ...
            if (round.winnerClaimed) {
                throw new Error(`Winner of round ${roundId} has already claimed the prize`);
            }

            // Update claimed status
            await this.kuroModel.findOneAndUpdate(
                { roundId, kuroContractAddress: this.configService.get<string>('KURO_ADDRESS') },
                {
                    winnerClaimed: true,
                    txClaimed: txHash
                }
            ...
            this.logger.error(`Error updating winner claimed status: ${error.message}`);
            throw error;
        }
    }
    ...
    async getPnL() {
        try {
            ...
            const latestPnl = await this.kuroModel.aggregate([
                {
                    $match: {
                    ...
                    $project: {
                        ...
                        pnl: {
                            $cond: {
                                ...
                                then: {
                                    $divide: ['$totalParticipantEntries', '$totalWinnerEntries'],
                                },
                                else: 0,
                            },
                        },
            ...
            ]).exec().then(result => result[0] || null)
