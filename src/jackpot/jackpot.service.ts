...
import { Jackpot, JackpotContribution, JackpotStatus } from "src/shared/schemas/jackpot.schema";

export class JackpotService {
    private readonly logger = new Logger(JackpotService.name);

    constructor(@InjectModel(JackpotContribution.name) private jackpotContributionModel: Model<JackpotContribution>,
    ...
    private async updateLastedJackpotRound(
        ...
        try {
            ...
            const _lastedJackpotRound = await this.jackpotModel.findOne().sort({ createdAt: -1 }).lean();
    ...
    async announceJackpotWinner(jackpotId: number, roundId: string, winner: string): Promise<Jackpot> {
        try {
    ...
    async recordJackpotContribution(roundId: string, jackpotFee: string, contributor: string): Promise<JackpotContribution> {
        try {
            const updatedJackpot = await this.updateLastedJackpotRound("", jackpotFee, "", "", "Processing");
            ...
            this.logger.error(`Error recording jackpot contribution: ${error.message}`);
            throw error;
        }
    }

    async randomJackpot(roundId: string, totalKuroRoundPool: string, contributor: string) {
        const lastedJackpotRound = await this.jackpotModel.findOne().sort({ createdAt: -1 }).lean();
        ...
        if (!lastedJackpotRound) {
            ...
            const winProbability = 0.0000015; // 0.15%
            if (randomValue < winProbability) {
                // win
                await this.announceJackpotWinner(lastedJackpotRound.jackpotId, roundId, contributor);
            }
            this.recordJackpotContribution(roundId, jackpotFee, contributor);
        }
    ...
    async lastedPoolData() {
        try {
            const currentJackpot = await this.jackpotModel.findOne().sort({ createdAt: -1 }).lean();
